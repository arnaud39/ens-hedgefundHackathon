...
def cfs_search(
    X_train_reshape: pd.DataFrame, Y_train: pd.DataFrame, size_search: int = 50
):
    A_full = randomA(F=size_search)
    # randomNormedA(D=250, F = 50)
    # define variables for cfs algorithm
    Y_candidate = Y_train.T.stack()
    y_df = pd.pivot_table(
        Y_candidate.reset_index(), index="date", values=0, columns="stocksID"
    )
    y_stack = [y_df[[k]] for k in range(50)]
...
def main():
    ...
    for iteration in tqdm(range(Niter)):
        ...
        m, A, beta = cfs_search(X_train_reshape, Y_train, int(args[0]))
        if m > maxMetric:
